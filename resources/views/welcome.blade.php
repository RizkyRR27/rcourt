<x-layouts.app>
    ...
    <div class="relative bg-black text-white py-24 px-6 text-center border-b-4 border-[var(--color-court-clay)]">
        ...
        @auth
            <a href="{{ route('booking') }}" class="inline-block bg-[var(--color-court-clay)] text-white px-8 py-4 font-mono font-bold uppercase border-2 border-white hover:bg-red-700 transition-all transform hover:scale-105 shadow-hard">
                Booking Sekarang 🚀
            </a>
        @else
            <div class="flex justify-center gap-4">
                <a href="{{ route('login') }}" class="bg-white text-black px-8 py-3 font-mono font-bold uppercase border-2 border-black hover:bg-gray-200 shadow-hard">
                    Masuk
                </a>
                <a href="{{ route('register') }}" class="bg-[var(--color-court-clay)] text-white px-8 py-3 font-mono font-bold uppercase border-2 border-white hover:bg-red-700 shadow-hard">
                    Daftar
                </a>
            </div>
        @endauth
    ...
    <div class="bg-[var(--color-court-paper)] border-b-2 border-black py-8">
        <div class="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            <div class="border-2 border-black bg-white p-4 shadow-hard">
                <h3 class="font-display text-4xl">{{ $totalCourts }}+</h3>
                <p class="font-mono text-xs uppercase text-gray-500">Lapangan Tersedia</p>
            </div>
            <div class="border-2 border-black bg-white p-4 shadow-hard">
                <h3 class="font-display text-4xl">{{ $totalMembers }}+</h3>
                <p class="font-mono text-xs uppercase text-gray-500">Member Aktif</p>
            </div>
            <div class="border-2 border-black bg-white p-4 shadow-hard">
                <h3 class="font-display text-4xl">{{ $totalBookings }}+</h3>
    ...
    <div class="py-16 px-4 bg-yellow-50">
        <div class="max-w-5xl mx-auto border-4 border-black bg-white p-8 md:p-12 shadow-hard-lg relative overflow-hidden">
            <div class="absolute top-0 right-0 bg-red-600 text-white font-mono font-bold px-12 py-2 transform rotate-45 translate-x-14 translate-y-6 border-2 border-black">
                HOT PROMO
            </div>
